...
const Home = () => {
  ...
  useEffect(() => {
   const query = useQuery(userInfo?.aud);
  ...
  return (
    <div className="flex bg-gray-50 md:flex-row flex-col h-screen transaction-height duration-75 ease-out">
      ...
      <HiMenu fontSize={40} className='cursor-pointer'  onClick={()=> setToggleSidebar(prev=> !prev  ) } />
      <NavLink to='/'>
      ...
      {toggleSidebar && (
        <div className="fixed w-4/5 bg-white h-screen overflow-y-auto shadow-md z-10 animate-slide-in">
          <div className="absolute w-full flex justify-end items-center ">
            <AiFillCloseCircle size={30} className="cursor-pointer " onClick={()=> setToggleSidebar(prev=> !prev  ) } />
          </div>
